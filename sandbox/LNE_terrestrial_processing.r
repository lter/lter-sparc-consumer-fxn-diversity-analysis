...
#### Read in ebird taxonomic data #### 
#want to read in e-bird taxonomic data to fill in family, order, etc. 
#ASK SHALANDA: is this better done by Li?
...
#focus on luquillo_abundance_EL_GT25 & luquillo_abundance_EL_LT25. see code below for more info about why.... 
...
#don't need to read in the four csvs below. see code below for more info about why.... 
...
## this is exploratory info about datasets #
...
## okay, now that we have identified the dataframes we care about, let's pivot
...
id_cols <- c("YEAR", "PLOT1", "PLOT2", "DATE", "PLACE")   # add others you have (SITE, DATE, TRANSECT, etc. on other datasets)
...
luquillo_birdspecieslist_formerge <- luquillo_abundance_EL_all_LONG_all_specieslist %>%
  mutate(project = "Luquillo",
         habitat = "terrestrial",
         raw_filename = "2filesmergedtogetherneedtofigureout", #obviously need to update this eventually...
         class = "Aves")

#okay now merge the species name csv with the common names from the data 
...
#maybe this is better done by Li?
...
#make a clean verison to write out -- this would be for trait work only, NO ABUNDANCE.
#should only have 42 observations. 
...
# I think fine? 
#now merge with abundance data... 

colnames(luquillo_birdspecieslist_formerge_taxo)
luquillo_abundance_EL_all_wrangled_step1 <- luquillo_abundance_EL_all_LONG %>%
  left_join(luquillo_birdspecieslist_formerge_taxo, by = c("species_code" = "species_code"))
  
#120290 obs! cool. 

# 
luquillo_abundance_EL_all_wrangled_step1 %>%
  dplyr::filter(is.na(scientific_name)) #missing some columns for unknown, unknown warbler, unknown thrasher, and total.
...
#### STACK TOGETHEER FOR SHALANDA BIRD TRAIT LIST ####
...
####SEV DATA - we are dropping this program right? ####
# sev ONLY HAS SPECIES CODES -- d
...
######################### skip this ################
# this was from when we were exploring biotime data, can skip 
